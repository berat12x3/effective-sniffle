...
module.exports = {
  name: 'gnewkey',
  description: 'Generate a new GITHUB key',
  async execute(message, args, client) {
    ...
    try {
      ...
      await db.query(
        `INSERT INTO gapi_keys (key, package_type, total_limit, used_count, created_at, expires_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [newKey, package_type, limit, 0, createdAt, expiresAt]
      );

      await message.reply(`GITHUB key generated: \`${newKey}\``);
      ...
      const logMessage = `**GITHUB KEY GENERATED**
• **Key:** \`${newKey}\`
...
• **Server:** \`${message.guild ? message.guild.name : 'DM'} (ID: ${message.guild ? message.guild.id : 'N/A'})\``;
      ...
      console.error('Error generating GITHUB key:', error);
